...
The `Rs3tools` package is maintained by other analysts in MoJ and has the advantage of being R-native, meaning it is simpler to install and use compared to `botor`. Here's how it can be installed:
...
The `botor` package provides an alternative method for reading from S3, which can be done using this command:
