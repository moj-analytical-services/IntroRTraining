...
Of course, you can also use google to try and find the solution to the problem. 
...
Note, you won't need to do this if you are working on the analytical platform but it's still a very useful command to know:
...
Once a package is installed, you should be able to see it in the packages tab. If you want to use it, you can load it by ticking the appropriate box in the packages window. You can also load packages using the library command:
...
The package tidyverse contains many useful packages such as dplyr which is a particularly useful package for manipulating and processing data. The package lubridate is very helpful when manipulating dates. Many of the functions in the rest of this training course are from these packages.
...
Alternatively (you have already loaded the readr package which is anyway part of the tidyverse package - see section 2.2) you can simply use the command read_csv():
...
To import the data for this session from the platform use this command:
...
As noted in the previous section, you can see by looking in the 'environment' window that the 'offenders' dataset has 1413 observations and 9 variables. To view this dataset, click the icon to the right of this information, which you can see from the console is the equivalent of using the command View(offenders)
...
Note the use of square brackets to specify which rows and columns you want, the rows being specified first and then the columns, the two being separated by a comma. c stands for concatenate, enabling us to retrieve the 500th and 502nd observations together.  
...
We can also use the select() command from the dplyr package to choose just the variables from the 'offender' dataset that we want:

```{r results="hide"}
Names <- select(offenders, Surname = LAST, FIRST)
```

Note here that we have created a new dataset called 'Names' and also that we have renamed the LAST variable as Surname.

To create a new variable providing the weight in kg (instead of it pounds):

```{r results="hide"}
offenders$weight_kg <- offenders$WEIGHT*0.454
```

The command mutate() which is part of the dplyr package can also be used to edit a variable or add a new variable while preserving the others.
 
```{r eval = FALSE}
?mutate
```

To use dplyr functions such as mutate, we need to enter a few bits of information into brackets immediately after the name of the function. First we specify the name of the dataset. Then we give a name to the new variable that we are going to create and then specify how it is going to be calculated. 
...
2.	Create a new dataset creating a new variable providing the number of (full) years over 16 and drop the orginal WEIGHT variable.
...
Factors are for categorical variables involving different levels. So for example, in the dataset 'offenders', FEMALE is stored as 1, and MALE as 2. We can see this now when looking at the environment tab (after clicking the arrow to the left of offenders) and also from using the following command:
...
Let's first take a look at the different possible values of the index_disposal variable. We can do that using the table command.

```{r}
table(offenders$SENTENCE)
```

To filter we just specify the data that we want to filter (offenders) and the value that we want to filter on. In this case lets filter where SENTENCE is "Court_order" and then recalculate the mean number of previous convictions with breakdown by REGION and GENDER:
...
1.  Using group_by and summarise, calculate the average age for females in the West.
...
There is a function called merge which can merge data sets, provided you have some common fields to match on. This is similar to joins in SQL.
...
For more information about the different sorts of joins see: http://www.dummies.com/programming/r/how-to-use-the-merge-function-with-data-sets-in-r/ 

We can also join two datasets vertically or horizontally, using the rbind or cbind functions respectively. 

If we have two datasets with the same variables, we can use "rbind" to join them vertically, R simply sticks them on top of each other. They actually only need to have the same number of variables but clearly, you need to be careful if you are joining data together which refers to different variables.
...
"cbind" does something similar but appends data horizontally, in this case you would need the same number of observations in each data frame. 
...
We can easily make a logical vector with value TRUE if the corresponding element is NA, and FALSE otherwise, using a funciton called "is.na":
...
Then we can use this to ascertain the number of missing values the height variable contains:
```{r}
table(Miss)
```
...
1.	Merge offenders_trial with the age column of offenders, creating a new dataset called offenders_trial_age.
2.	Export the dataset offenders_trial_age to a csv file.
3.	Using offenders create a new variable HEIGHT_NEW which is as HEIGHT except with the missing values replaced by the average height. 
4.	Produce a table showing the counts of HEIGHT including missing values. 
