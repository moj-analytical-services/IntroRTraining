...
[Introduction] - in this section you will be introduced to R, the RStudio Environment and some of the basic R commands

[Processing data] - this section introduces R packages, importing data into R,  inspecting data and data classes and the ifelse function

[Data wrangling and 'group by' calculations] - this section covers the most useful data manipulation functions using the dplyr package

[Dates] - this section explores working with dates

[Merging data, missing values and exporting] - This section explores some more advanced data wrangling techniques, as well as exporting data
...
## Running code in this RMarkdown training file

When you want to run code in this training RMarkdown file you have more options to run the code. One is to put your cursor in a line of code you want to run then click the Run button top right. You'll see this menu appear and different ways to run the code you can experiment with.

Two other ways to run your code only using the keyboard are:

1. Put your cursor anywhere on a line of code you want to run then hold down the Ctrl key and hit Enter.

2. Block on to one or more lines of code you want to run then hold down the Ctrl key and hit Enter.
...
You can also block on to a function name in the code and hit the F1 key. This also launches the right help page.

Of course, you can also look for the solution to your problem using Google, the ASD Slack Workspace that has an #r channel: https://asdslack.slack.com/ Or the Government Data Science #r channel: https://govdatascience.slack.com/ Don't be afraid to ask a question you think is too basic on these Slack channels. Everyone is supportive for questions at any level.
...
Packages extend R's functionality enormously and are a key factor in making R so popular. For instance, to install the tidyverse package in R you can do it one of three ways:

1. From the Packages tab in the bottom right pane of Rstudio click the "Install"" button.
2. From the Tools menu select the "Install Packages..." option.
3. Install the package using code as shown below:

```{r eval = FALSE}
# install.packages("tidyverse")
```

Once a package is installed, you should be able to see it in the packages tab in the bottom right pane of RStudio. If you want to use it, you can load it by ticking the appropriate box in the packages window. You can also load packages using the library command, which you can inside your script, so they will automatically load when you run it:
...
Alternatively (you have already loaded the readr package as it is part of the tidyverse package - see the [Packages] section) you can simply use the command read_csv():
...
To obtain a summary of the characteristics of your dataset you can click on the arrow by 'offenders' in the 'environment' window, which provides the same information as by typing the following command:

```{r, results="hide"}
str(offenders)
```

Looking at the output provided informs you that the dataset 'offenders' is in R terminology both a tibble and a dataframe, and as we've already seen has 1413 observations and 9 variables. Variables in a data frame are like columns in a table, and are stored as vectors.

Also provided is some information about each variable (or vector) in the dataset (or dataframe) as designated by R; the name, the type (in this case either integer, number or character).  
...
Because R data is stored as vectors we can isolate or subset elements of it using square brackets. This is much like we can isolate elements of an Excel sheet using cell references like A1:A12. For example, 'offenders[ i , j ]' would return the value in the ith row and jth column of the dataframe 'offenders'. So, if you want the fourth variable for the 500th observation:
...
```{r results="hide"}
offenders$wt_under_90  <- base::ifelse(offenders$weight_kg<90, 1, 0)
```
...
In the above code R is taking the 'offenders' dataset, grouping it first by REGION and then by GENDER (note that the data is "piped" like water into the dplyr::group_by command using the pipe symbol '%>%' instead of specifying the 'offenders' dataset as the first argument within the dplyr::group_by command), and then outputting the mean number of previous convictions by REGION and GENDER. The new mean number of previous convictions variable we've decided to call 'Ave'. The results are saved into a new dataset called 'regional_gender_average'.  
...
The regional_gender_average dataset is grouped by REGION and GENDER. If we run it through dplyr::sumuraise again the result will be grouped by the first grouping variable set which is REGION For example, the dplyr:summarise() function applied below counts the rows in each of the first group by category.
...
But if we wanted to count all the rows in the dataset with the grouping removed we add in the dplyr::ungroup() function as below:
...
offenders_anonymous <- offenders %>%
  dplyr::select(-LAST, -FIRST, -BLOCK, -BIRTH_DATE)

head(offenders_anonymous,10)
```
...
In the section [Ifelse] we created a new column using the base::ifelse() fuction from Base R. Functions in Base R are always available and don't require you to install a package and then call the library for that package. Here is a good <a href="https://www.rstudio.com/wp-content/uploads/2016/05/base-r.pdf" target="_blank">Base R cheatsheet</a> of the base functions you'll probably use a lot.

A clear way to code if-then-else logic within dplyr::mutate() is dplyr's <a href="https://dplyr.tidyverse.org/reference/case_when.html" target="_blank">case_when()</a>. It lets you code complex logic that is easy to read. For example, below we add a new more complex categorised column for weight called weight_category.
...
offenders_anonymous <- offenders %>%
  ...
  dplyr::mutate(weight_kg = WEIGHT * 0.454,
                weight_category = 
                  dplyr::case_when(weight_kg >= 0.0 & weight_kg < 70.0    ~ 'low weight',
                                   weight_kg >= 70.0 & weight_kg <= 90.0  ~ 'medium weight',
                                   TRUE                                   ~ 'heavy weight'))

head(offenders_anonymous,10)
```
Note how uesful it is that the case_when() is able to use the weight_kg column that was created earlier in the same dplyr::mutate(). The "TRUE" statement at the end of the case_when() means the the same thing as "else" in a convenatoinal if-then-else code.

For more help on dplyr, download the <a href="https://www.rstudio.com/resources/cheatsheets/#dplyr" target="_blank">Data Transformation Cheat Sheet</a>  for dplyr mfor more guidance. RStudio creates several more useful <a href="https://www.rstudio.com/resources/cheatsheets/" target="_blank">Cheat Sheets</a> on several fundamental topics in R. You can also access all their cheat sheets from the Help menu in RStudio under "Cheatsheets"

And here is a very well explained <a href="https://suzanbaert.netlify.com/2018/01/dplyr-tutorial-1/" target="_blank">dplyr tutorial</a> covering all the dplyr topics here (and many more) to an advanced level.
...
In all of this section, we are going to use a package from tidyverse designed called <a href="https://resources.rstudio.com/the-essentials-of-data-science/lubridate-cheat-sheet" target="_blank">Lubridate</a>, there are also ways to do the same thing using base R. First, we need to load the package:
```{r}
library(lubridate)
```
...
```{r}
offenders<- dplyr::mutate(offenders, DoB_formatted = lubridate::mdy(BIRTH_DATE))
...
```{r results="hide"}
offenders <- dplyr::mutate(offenders, day = lubridate::day(DoB_formatted))

offenders <- dplyr::mutate(offenders, quarter = lubridate::quarter(DoB_formatted))

offenders <- dplyr::mutate(offenders, year = lubridate::year(DoB_formatted))

offenders <- dplyr::mutate(offenders, month = lubridate::month(DoB_formatted))

offenders <- dplyr::mutate(offenders, weekday = base::weekdays(DoB_formatted))
```
...
```{r results="hide"}
offenders <- dplyr::mutate(offenders, days_before_2000 = lubridate::ymd("2000-01-01") - DoB_formatted)
```
...
For instance:
```{r results="hide"}
men <- dplyr::filter(offenders, GENDER == "MALE")
women <- dplyr::filter(offenders, GENDER == "FEMALE")
...
Another common data manipulation task is to reshape data from wide to long or from long to wide. A common reasons for reshaping are to make your data easier to visualise in a chart or table, or to let you perform calculations on the data with less code.

Let's use the exampe of aggegrating then reshaping the data to make a table that is easier to read.

```{r}
offenders_pretty_table <- offenders %>%
  dplyr::group_by(GENDER,REGION) %>%
  dplyr::summarise(n = n())

offenders_pretty_table
```
This could be easier for people to read and compare values between categories if they could see each REGION value as seperate columns like this:
...
Or perhaps the values in GENDER as seperate columns.
...
So this is reshaping data from long to wide using tidyr::spread. We can also reshape from wide to long using tidyr::gather. Below we create the wide table first, then we reshape it from wide to long.

```{r}
offenders_wide <- offenders_pretty_table %>%
  tidyr::spread(key = GENDER, value = n)
...
```{r}
offenders_long <- offenders_wide %>%
  tidyr::gather(key = REGION, value = "GENDER")

offenders_long
```

The explanation of how to reshape with tidyr on page2 of this <a href="https://resources.rstudio.com/tidyverse/data-import-cheat-sheet" target="_blank">cheat sheet extract</a> shown below are excellent.
...
+ DataCamp is a website which hosts multiple online courses that teach coding. Their <a href="https://www.datacamp.com/courses/free-introduction-to-r" target="_blank">Introduction to R course</a> is free to complete and provides a broader overview in the basic concepts for coding in R.

+ Another good resourse is the <a href="https://r4ds.had.co.nz/" target="_blank">R for Data Science</a> online book written by Hadley Wickham, who is a data scientist at RStudio. He developed the tidyverse package that we introduced earlier. It gives a really good overview of R and how his package works with it.

+ RStudio has also developed a list of <a href="https://www.rstudio.com/resources/cheatsheets/" target="_blank">Cheat Sheets</a> which give quick overviews of the functions contained in different packages, which can be quickly referred to. Some can be accessed directly through the top menu help > Cheatsheets e.g. 'Data Transformation with dplyr'

For further lists of useful resources please see this <a href="https://trello.com/b/D5pSkqnT/online-analytical-training" target="_blank">Trello Board of Online Training</a>