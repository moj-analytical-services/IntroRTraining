...
The regional_gender_average dataset is grouped by REGION and GENDER. So if we run it through dplyr::sumuraise agin the result will be by the first grouping variable set which is Region. For example, the code below counts the rows per group by category.

```{r}
row_count <- regional_gender_average %>% 
  dplyr::summarise(n = n())

row_count
```

But if we wanted to remove this grouping and count all the rows in the dataset with group ing remvoed we would add dplyr::ungroup() as below:
