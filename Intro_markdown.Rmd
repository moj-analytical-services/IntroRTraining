...
Dataframes/tibbles in R are a collection of vectors where where each vector is a column and represents a variable. To view a specific variable, for instance gender, you can use a dollar sign as follows:
...
The pipe operator simply passes through the object on the left hand side as the first argument of the following fuction. For example the above calculation without using the pipe operator would look like this:
...
Using nested functions this looks more complicated and is harder to follow than if we use the pipe operator, this becomes more clear as extra calculations are added. 
