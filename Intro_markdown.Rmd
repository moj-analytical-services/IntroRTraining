...
The regional_gender_average dataset is grouped by REGION and GENDER. If we run it through dplyr::sumuraise again the result will be grouped by the first grouping variable set which is REGION For example, the summarise() function applied below counts the rows in each of the first group by category.
