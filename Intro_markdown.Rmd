...
Let's make another object which is a vector with the values 3, 2 and 4.

```{r}
x <- c(3, 2, 4)
```

c represents concatenate. You can now see the new object x in the 'workplace'. Note that the old object x has been overwritten and that the new object is of class numeric. 
...
If you want the fourth and fifth variables for the 500th observation:

```{r}
offenders[500,4:5]
```

The colon enables us to retrieve from the 4th to the 5th columns.  
...
Using these functions together we can replicate the apply family of functions in base R and produce summary statistics in a similar way to pivot tables in Excel. We can use these functions together using the pipe (%>%) operator which makes code more readable
and means you don't have to create a new object each time you run a command.
...
As you might have noticed, DoB in the 'offenders' dataset currently has a class of character (or factor depending how you read the data in). To be able to manipulate this data as a date in R, we first need to convert it to class date.
...
If you have a read of the help file `r ?lubridate`, you'll see Lubridate has a number of functions such as dmy(), myd() etc whose name models the order in which the year ('y'), month ('m') and day ('d') elements appear in the string to be parsed.
...
The function mdy() specifies the format that the date in column DoB is currently in so R knows where to find the day, month and year needed to create a date. 
...
3.	Work out which day of the week has the highest mean performance using summarise(). 
...
+ Datacamp is an website which hosts multiple online courses that teach coding. Their 'Introduction to R' course is free to complete and also provide a good overivew in the basic concepts for coding in R. A link to the course can be found here: [https://www.datacamp.com/courses/free-introduction-to-r](https://www.datacamp.com/courses/free-introduction-to-r)
...
+ RStudio has also developed a list of 'cheatsheets' which give quick overviews of the functions contained in different packages, which can be quickly referred to: [https://www.rstudio.com/resources/cheatsheets/](https://www.rstudio.com/resources/cheatsheets/)
